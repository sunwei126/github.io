...
```{r echo=FALSE, message=FALSE, warning=FALSE}
options(scipen= 200)
qplot(data = df.lc, x =借款金额, fill = I('#F79420'))+
  xlim(0, quantile(df.lc$借款金额, 0.99))+
  scale_x_log10()
summary(df.lc$借款金额)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
qplot(x= 历史成功借款金额, data = df.lc)+
xlim(0, quantile(df.lc$历史成功借款金额, 0.99))+
...
存在异常分布的3个变项为：借款金额、历史成功借款次数和历史成功借款金额；在制作图表的时候，
按照分布在99%范围的数据进行显示，避免对大多数情况下的展示有影响；但原始数据不做调整
...
双变项分析：8）在所有的验证方式总数的基础上，随着验证数的增加，借款金额增加
...
双变项分析：9）随着初始评级的下降，逾期本金金额和逾期金额占比有一定提高
...
双变项分析：10）初始评级为C和D的借款标的逾期情况明显高于其它评级，D略高于C
...
双变项分析：11）2016年1月以后发行的标，逾期金额占比普遍明显增多
...
双变项分析：12）2016年1月之后成功借款的标的,逾期金额占比借款金额快速升高。
...
8）在所有的验证方式总数的基础上，随着验证数的增加，借款金额增加

9）随着初始评级的下降，逾期本金金额和逾期金额占比有一定提高

10）初始评级为C和D的借款标的逾期情况明显高于其它评级，D略高于C

11）2016年1月以后发行的标，逾期金额占比普遍明显增多

12）2016年1月之后成功借款的标的,逾期金额占比借款金额（逾期率）快速升高。
...
ggplot(aes(x = 历史成功借款金额, y = 借款金额, color= 初始评级), data = df.lctrack_new) + 
  geom_jitter(alpha = 1/20, size = 1, position = 'jitter') +
  scale_x_log10()+
  scale_y_log10()+
  geom_smooth()
  #scale_y_continuous(limits = c(0,1), breaks = seq(0,1,0.1)) +
  scale_color_brewer(type="div", guide = guide_legend(title = '初始评级', reverse = F,
                                          override.aes = list(alpha = 1, size = 2)))
...
ggplot(aes(x = 历史成功借款金额, y = 借款金额, color= 借款类型), data = df.lctrack_new) + 
  ...
  geom_smooth()
  #scale_y_continuous(limits = c(0,1), breaks = seq(0,1,0.1)) +
  scale_color_brewer(type="div",guide = guide_legend(title = '性别', reverse = F,
                                          override.aes = list(alpha = 1, size = 2)))
...
ggplot(aes(x = 历史成功借款金额, y = residual_principle/借款金额, color= 借款类型), data = df.lctrack_new) + 
  ...
  geom_smooth()+
  scale_y_continuous(limits = c(0,1), breaks = seq(0,1,0.1)) +
  scale_color_brewer(type="div",guide = guide_legend(title = '借款类型', reverse = F,
                                          override.aes = list(alpha = 1, size = 2)))
```
...
ggplot(aes(x = 历史成功借款金额, y = 借款金额, color= age.bucket), data = df.lctrack) + 
  ...
  scale_y_log10()+
  #scale_y_continuous(limits = c(0,1), breaks = seq(0,1,0.1)) +
  scale_color_brewer(type="div",guide = guide_legend(title = '年龄分组', reverse = F,
                                          override.aes = list(alpha = 1, size = 2)))
```
...
2）在2016年1月前，各初始评级的逾期金额占比分布相对正常，之后初始评级失效

3）在2015-01到2017-01发行的标的，首标的平均借款金额逐渐超过非首标的平均借款金额

4）首标的逾期金额占比始终高于非首标的逾期金额占比

5）在2016-01之后，未进行认证的顾客显著增加

6）在2016-01之后，各个认证成功数的标的均出现逾期金额占比高企的情况

7)在历史借款金额越高，借款金额越高的基础上，B级的跨度区间最大，C级和D级标的金额和历史借款金额高度集中

8）在2016-01之前,逾期金额占比：其他>普通>电商,2016-01之后变化剧烈

9)在历史借款金额越高，借款金额越高的基础上，B级的跨度区间最大，C级和D级标的金额和历史借款金额高度集中

10）A-E初始评级的的逾期率均是先上升后下降

11）APP闪电贷和普通贷款分布最多，且金额集中

12）APP闪电、普通和其他贷款逾期金额占比,随着历史成功借款金额增多而增多；而电商贷款是随着历史成功借款金额的增多而减少

13）两个年龄层次是2015-01至2017-01时间范围内占到标的的主体，(16，26]和(26,36],其中16-26为大学生群体

14）各年龄层次是2015-01至2017-01时间范均随着历史成功借款金额增大，逾期率下降；历史借款金额低于1000元情况下，（16，26]年龄组逾期率最低

15）作为首标的借款金额均会与低于10万元，且普遍利率要低于非首标的情况

16）大部分情况下首次标的的逾期金额占比借款金额要低于非首标的情况
...
2)根据数据推论出初始评级/信用评级在2016年2月开始发行的标的出现失效的状态，这可能与行业环境以及特殊借款群体有关
...
1.数据导入过程中，含有中文字符的CSV文件导入

2.原始数据中包含多组数据，难以短时间内理解各字段的含义和联系

3.从单变量分析到双变量分析，再到多变量分析；无法快速找到研究重点字段对象

二、分析过程中成功的发现部分

1.历史成功借款金额与借款金额的高度相关性

2.2016年1月之前和之后逾期金额占比的巨大变化

3.各种不同贷款类型在不同时间段的分布情况变化

4.各种不同初始评级贷款不同时间段的分布情况变化

三、未来如何进一步丰富分析内容和提高报告质量，以下为可以提升的几点需进行进一步研究
